...
# class CUB200DataModule(nn.Module):
#     def __init__(self,
#                  pretrain_batch_size: int = 64,
#                  other_batch_size: int = 32,
#                  download: bool = False,
...
class TinyImageNetDataModule(nn.Module):
    def __init__(self,
    ...
    def prepare_data(self):
        ...
        wnids = [wnid.rstrip('\n') for wnid in open('/'.join([self.dataset_path,'tiny-imagenet-200','wnids.txt']),'r').readlines()]

        label_dict = dict(zip(wnids,list(range(200))))

        self.train_df = pd.DataFrame(columns = ['filename', 'label'])
        self.valid_df = pd.DataFrame(columns = ['filename', 'label'])
        self.test_df = pd.DataFrame(columns = ['filename', 'label'])

        for wnid in wnids:
            filepath = '/'.join([self.dataset_path, 'tiny-imagenet-200', 'train', wnid, 'images'])
            files = os.listdir(filepath)
            for f in files:
                self.train_df = self.train_df.append({'filename':'/'.join([filepath,f]),'label':label_dict[wnid]}, ignore_index = True)

        self.train_df, self.valid_df = train_test_split(self.train_df, test_size = 0.2)

        val_annotations = open('/'.join([self.dataset_path, 'tiny-imagenet-200', 'val','val_annotations.txt']), 'r').readlines()
        val_annotations = [val_ann.rstrip('\n').split('\t') for val_ann in val_annotations]
        val_labels = [label_dict[val_ann[1]] for val_ann in val_annotations]
        val_files = [val_ann[0] for val_ann in val_annotations]

        # files = os.listdir('/'.join([self.dataset_path, 'tiny-imagenet-200', 'val','images']))
        
        for i in range(len(val_files)):
            filepath = '/'.join([self.dataset_path, 'tiny-imagenet-200', 'val','images',val_files[i]])
            self.test_df = self.test_df.append({'filename':filepath, 'label':val_labels[i]}, ignore_index = True)

        # files = os.listdir('/'.join([self.dataset_path,'test','images']))
        # for f in files:
        #     valid_df = valid_df.append({'filepath':'/'.join([filepath,f]),'label':0}, ignore_index = True)
    ...
    def setup(self, stage: Optional[str] = None, pretrain : Optional[bool] = True, fracs: Optional[float] = 1.0):
        ...
        if stage == 'train':
            ...
            if pretrain:
                ...
                self.traingen = SLDataFrameDataset('train', self.train_df, self.MEAN, self.STD, transforms.RandomResizedCrop(64, (0.8,1.0)), fracs)
